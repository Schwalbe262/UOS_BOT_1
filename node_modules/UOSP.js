...
// 장학 공지
UOSP.UOSP5 = function UOSP4(){
    let str = ""
    let SW_new = 0; // 새로운 공지가 파싱되었을 경우 1이됨

    let title = org.jsoup.Jsoup.connect("https://scholarship.uos.ac.kr/scholarship/main.do").get().select(".main_notice>ul>.main_notice_news>.f_left>a").get(0).ownText()
    ...
    let pre_DB = temp[temp.length-1] // 이전 DB 마지막 부분
    if ( pre_DB != String(title).replaceAmp()) { // 새로운 내용이 파싱되었을 경우
        temp.push(title)
        UOSP.setDB("UOSP5_DB",temp)
        SW_new = 1
    }
