...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        ...
        try{
            ...
            DB_adress = Math.floor(23456/20000) // 해당 데이터가 저장될 DB 번호호
        ...
        if(i%1000==0){
            if(count_sucess2/(count_sucess2+count_failed2)>0.5){
...
DCP.UOS_list = function UOS_list(room){
    ...
    let temp = JSON.parse(DCP.getDB("UOS_DB_"+Number(DCP.getDB("UOSDB_index"))))
