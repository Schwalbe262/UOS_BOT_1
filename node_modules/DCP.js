...
DCP.UOSP = function UOSP(){ // 시갤 파싱
    ...
    if ( pre_DB[1] < adress) { // 새로운 내용이 파싱되었을 경우
        Api.replyRoom("봇장난","UOS_collect 실행 : "+Number(Number(String(pre_DB[1]))+1)+"부터 "+Number(adress)+"까지")
        DCP.UOS_collect( Number(Number(String(pre_DB[1]))+1) , Number(adress) ) // DB 기록작업
        SW_new = 1
    }
