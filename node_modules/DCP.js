...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        ...
        try{
            ...
            DB_adress = Math.floor(i/20000) // 해당 데이터가 저장될 DB 번호호
...
DCP.UOS_search = function UOS_search(key,mod){
    ...
    if(mod=="title"){
        ...
        for(let i=Number(DCP.getDB("UOSDB_index")) ; i>-1 ; i--){
            temp = temp + JSON.parse(DCP.getDB("UOS_DB"+i)).filter(v=>String(v[0]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0})
                .map(v=>"제목 : "+v[0]+"\n작성자 : "+v[3]+" "+v[4]+"\n날짜 : "+v[2]+"\n- https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("")
        }
        ...
        let temp = ""
        for(let i=Number(DCP.getDB("UOSDB_index")) ; i>-1 ; i--){
            temp = temp + JSON.parse(DCP.getDB("UOS_DB"+i)).filter(v=>String(v[3]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0})
                .map(v=>"제목 : "+v[0]+"\n작성자 : "+v[3]+" "+v[4]+"\n날짜 : "+v[2]+"\n- https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("")
        }
        return temp.trim().substr(0,200000)
    }
        ...
        let temp = ""
        for(let i=Number(DCP.getDB("UOSDB_index")) ; i>-1 ; i--){
            temp = temp + JSON.parse(DCP.getDB("UOS_DB"+i)).filter(v=>String(v[4]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0})
                .map(v=>"제목 : "+v[0]+"\n작성자 : "+v[3]+" "+v[4]+"\n날짜 : "+v[2]+"\n- https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("")
        }
        return temp.trim().substr(0,200000)
    }
