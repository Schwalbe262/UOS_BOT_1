        ...
        if(i%1000==0){
            if(count_sucess2/(count_sucess2+count_failed2)>0.3){
                ...
                Api.replyRoom(console_room_name,"임시 DB 저장 완료\n성공 : "+count_sucess2+"\n실패 : "+count_failed2+
                    "\n경과시간: " + java.lang.String.format("%.2f",DCP.timer.end()/1000) + "초"[pdr rr])
                DCP.timer.start()
                count_sucess2 = 0
                count_failed2 = 0
            } // 백업
...
DCP.UOS_stat = function UOS_stat(){
    ...
    for(let i=Number(DCP.getDB("UOSDB_index")) ; i>-1 ; i--){
        for(let j=2006 ; j<2021 ; j++)
            num[j] = num[j] + JSON.parse(DCP.getDB("UOS_DB"+i)).map(v[4]).includes(String(j)).length
    }

    let str = ""
    for(let j=2006 ; j<2021 ; j++){
        str = str + String(j) + "년 : " + num[j] + "개\n"
    }
