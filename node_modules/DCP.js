...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        ...
        try{
            ...
            pre_DB.push(array)
            count_sucess++
            java.lang.Thread.sleep(2000*Math.random()+1000) // 200ms
        }
        catch(e){count_failed++} // 해당 주소에 아무런 내용이 존재하지 않을 경우 아무것도 하지 않음


        if(i%50==0){
            Api.replyRoom(console_room_name,i+"번 주소까지 진행 완료\n"+"성공 : "+count_sucess+"\n실패 : "+count_failed+"경과시간: " + java.lang.String.format("%.2f",DCP.timer.end()/1000) + "초")
...
DCP.UOSP_list = function UOSP_depart_list(room){
