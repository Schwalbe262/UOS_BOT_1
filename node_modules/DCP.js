...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        ...
        try{
            if(break_var != 0){break}

            java.lang.Thread.sleep(200*Math.random()+100) // 200ms
        ...
        if(i%1000==0){
            if(count_sucess2/(count_sucess2+count_failed2)>0.65){
                DCP.setDB("UOS_DB_temp",JSON.stringify(pre_DB))
...
DCP.UOS_list = function UOS_list(room){
    ...
    for(let i=0 ; i<5 ; i++){
        try{
            title[i] = temp[4-i][0]
            sub_title[i] = temp[4-i][3]+temp[4-i][4] + " | " + temp[4-i][2].split(" ")[0]
...
DCP.apply_backup = function apply_backup(){
    ...
    DCP.setDB("UOS_DB_temp",JSON.stringify(a))
}
