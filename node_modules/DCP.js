...
DCP.UOS_parsing = function UOS_parsing(){
    Api.replyRoom("봇장난","UOS_parsing실행")
    DCP.UOSP()
}
...
DCP.UOSP = function UOSP(){ // 시갤 파싱
    ...
    if ( pre_DB[1] < adress) { // 새로운 내용이 파싱되었을 경우
        let num_a = Number(Number(String(pre_DB[1]))+1)
        let num_b = Number(adress)
        Api.replyRoom("봇장난","UOS_collect 실행 : "+num_a+"부터 "+num_b+"까지")
        DCP.UOS_collect( num_a , num_b ) // DB 기록작업
        SW_new = 1
    }

    if (SW_new == 1) {
        
        Api.replyRoom("봇장난","카링 보내기 전")
        ...
        Api.replyRoom("시갤톡방",title)

        Api.replyRoom("봇장난","카링 전송 완료")
    }
}
